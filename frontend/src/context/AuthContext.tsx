...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const savedToken = sessionStorage.getItem('novax_token');
    if (savedToken) {
      setToken(savedToken);
      userApi.getProfile()
        .then(setUser)
        .catch(() => { sessionStorage.removeItem('novax_token'); })
  ...
  const login = async (ci_or_correo: string, password: string) => {
    ...
    sessionStorage.setItem('novax_token', data.token);
    setToken(data.token);
    setUser(data.user);
  };
  ...
  const logout = () => {
    sessionStorage.removeItem('novax_token');
    setToken(null);
    setUser(null);
    router.push('/');
  };
